...
//
float Fhan_ADRC(float e1,float e2,float r,float h)
{
  ...
  fh = -r*(a/d)*Fsg_ADRC(a,d)-r*Sign_ADRC(a)*(1-Fsg_ADRC(a,d));
  return fh;
}

//ADRC最速跟踪微分器TD，使用fhan
void TD_ADRC(Fhan_Data *fhan_Input,float expect_ADRC)//安排ADRC过渡过程
{
  fhan_Input->fh = Fhan_ADRC((fhan_Input->x1-expect_ADRC),fhan_Input->x2,fhan_Input->r,fhan_Input->h0);
...
//第二套
  float fal_output=0;
  if(ABS(e)<=zeta)
    fal_output=e/powf(zeta,1-alpha);
  else
    fal_output=powf(ABS(e),alpha)*Sign_ADRC(e);
  return fal_output;
}
...
void Nolinear_Conbination_ADRC(Fhan_Data *fhan_Input)
{
  ...
  float temp_e2=0;
  temp_e2=Constrain_Float(fhan_Input->e2,-3000,3000);
  fhan_Input->u0 = -Fhan_ADRC(fhan_Input->e1,(fhan_Input->c*temp_e2),fhan_Input->r,fhan_Input->h1);
}
...
float ADRC_Control(Fhan_Data *fhan_Input,float expect_ADRC,float feedback_ADRC)
{
  ...
  fhan_Input->u=Constrain_Float(fhan_Input->u,-200,200);//加入扰动补偿
  return fhan_Input->u;
}
