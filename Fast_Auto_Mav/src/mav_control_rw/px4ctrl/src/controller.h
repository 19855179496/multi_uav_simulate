...
public:
  ...
  quadrotor_msgs::Px4ctrlDebug MpcControl(const Desired_State_t &des,
                                                  const Odom_Data_t &odom,
                                                  const Imu_Data_t &imu,
                                                  Controller_Output_t &u);
  ...
  Eigen::Vector3d fromQuaternion2euler(Eigen::Quaterniond q);
...
private:
  ...
  //轨迹队列
  std::vector<Desired_State_t> des_state_vec_;
