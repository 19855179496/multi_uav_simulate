...
//                                        &MavControlInterfaceImpl::OdometryWatchdogCallback, this, false, true);
  ...
  //订阅imu
  imu_subscriber_ = nh_.subscribe("/mavros/imu/data", 1, &MavControlInterfaceImpl::ImuCallback, this);
...
void MavControlInterfaceImpl::OdometryCallback(const nav_msgs::OdometryConstPtr& odometry_msg)
{
  ...
  static mav_msgs::EigenOdometry last_odometry;
  ...
  mav_msgs::eigenOdometryFromMsg(*odometry_msg, &last_odometry);
...
{
    std::cout << "OdometryWatchdogCallback" << std::endl;
  state_machine_->process_event(state_machine::OdometryWatchdog());
}

void MavControlInterfaceImpl::ImuCallback(const sensor_msgs::ImuConstPtr& msg)
{
  ROS_INFO_ONCE("Control interface got first IMU message.");
  sensor_msgs::Imu imu;
  imu = *msg;
  imu.header.stamp = ros::Time::now();
  state_machine_->process_event(state_machine::ImuUpdate(imu));
}
