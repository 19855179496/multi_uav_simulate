...
struct ImuUpdate
{
  ImuUpdate(const sensor_msgs::Imu& _imu)
      : imu(_imu)
  {
  }
...
//fsm 是 StateMachineDefinition的实例
class StateMachineDefinition : public msm_front::state_machine_def<StateMachineDefinition>
{
 ...
 public:
  ...
  struct transition_table : boost::mpl::vector<
      ...
      //imu_updata
      msm_front::Row<Inactive, ImuUpdate, InternalTransition, SetImu, NoGuard >,
...
//      msm_front::Row<PositionHold, OdometryUpdate, InternalTransition, SetOdometry, NoGuard>,
      msm_front::Row<PositionHold, OdometryUpdate, InternalTransition, SetOdometryAndCompute, NoGuard>,
      //imuset
      msm_front::Row<PositionHold, ImuUpdate, InternalTransition, SetImu, NoGuard >,
...
private:
  ...
  struct SetReferencePosition
  {
    ...
    {
        //打印array_size
      //ROS_INFO("array_size: %d", evt.references.size());
...
//set_imu
  ...
  {
    template<class FSM, class SourceState, class TargetState>
    void operator()(const ImuUpdate& evt, FSM& fsm, SourceState&, TargetState&)
    {
      fsm.controller_->setImu(evt.imu);
    }
  };
