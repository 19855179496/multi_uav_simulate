      ...
      queue_start_time_(0.0)
{
  ...
  //publish at 10 hz
  publish_queue_marker_timer_ = nh_.createTimer(ros::Duration(0.01), &MPCQueue::publishQueueMarker, this);
...
void MPCQueue::initializeQueue(const mav_msgs::EigenOdometry& odometry,
	...
	point.velocity_W = odometry.getVelocityWorld(vel_in_body_frame_);
...
{
  //打印current_queue_size_
  //std::cout << "current_queue_size_: " << current_queue_size_ << std::endl;
  //std::cout << "minimum_queue_size_: " << minimum_queue_size_ << std::endl;
...
void MPCQueue::pushBackPoint(const mav_msgs::EigenTrajectoryPoint& point)
{
  if (current_queue_size_ < maximum_queue_size_) {
    ...
    current_queue_size_++;
    // std::cout << "current_queue_size_: " << current_queue_size_ << std::endl;
    // std::cout << "maximum_queue_size_: " << maximum_queue_size_ << std::endl;
  } else {
