...
nh.param("bikrrt/allow_close_goal", allow_close_goal_, true);
  ...
  nh.param("bikrrt/run_in_sim", run_in_sim_,false);
                      ...
                      double search_time)
{
  ...
  if (bvp_.solve(start_node_->x, goal_node_->x, ACC_KNOWN))
  {
    ...
    if (vel_cons && acc_cons && jerk_cons && pos_cons)
    {
      ...
      final_traj_use_time_ = time_get() - t_start_;
...
{ 
  ...
  double rrt_start_time = time_get();
  double first_goal_found_time, final_goal_found_time;
  ...
  int idx = 0;
  for (idx = 0; (time_get() - rrt_start_time) < search_time && valid_start_tree_node_nums_ < n ; ++idx) 
  {
          ...
          if (connected && sampled_node_goal_tree->cost_from_start + bvp_.getCostStar() < curr_node->cost_from_start) 
          {
            ...
            if (test_convergency_ && goal_found)
            {
              if (curr_traj_node_set_goal_tree.count(curr_node) > 0) // rewire a node in curr best traj
              {
                ...
                double curr_goal_found_time = time_get();
                double curr_traj_use_time_ = time_get() - t_start_;
                Trajectory traj;
    ...
    if (sampled_node_goal_tree != nullptr && sampled_node_start_tree != nullptr) 
    {
      ...
      if (first_time_find_goal) 
      {
        ...
        first_goal_found_time = time_get();
        first_traj_use_time_ = first_goal_found_time - t_start_;
  ...
  if (goal_found) 
  {
    final_traj_use_time_ = t_end_ - t_start_;
  ...
  else if ((time_get() - rrt_start_time) >= search_time)
  {
    ROS_ERROR_STREAM("[BIKRRT]: NOT CONNECTED TO GOAL after " << (time_get() - rrt_start_time) << " seconds");
    ROS_INFO_STREAM("[BIKRRT]: total sample times: " << idx);
    ROS_INFO_STREAM("[BIKRRT]: valid sample times: " << valid_sample_nums_);
    ROS_INFO_STREAM("[BIKRRT]: valid tree node nums: " << valid_start_tree_node_nums_);
    return FAILURE;
