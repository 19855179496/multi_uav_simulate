...
double cmd_pub_rate = 100;
double cmd_duration = 1 / cmd_pub_rate;
ros::Duration cmd_pub_duration(cmd_duration);
...
void odomCallbck(const nav_msgs::OdometryConstPtr odom)
{
  ...
  curr_position = _curr_posi;
  curr_q.w() = odom->pose.pose.orientation.w;
  curr_q.x() = odom->pose.pose.orientation.x;
  curr_q.y() = odom->pose.pose.orientation.y;
  curr_q.z() = odom->pose.pose.orientation.z;
...
//记录cmd数据的最大值
void cmdrecord(quadrotor_msgs::PositionCommand &_cmd)
{
  double vel,acc;
  static double max_vel,max_acc;
  vel = sqrt(_cmd.velocity.x * _cmd.velocity.x + _cmd.velocity.y * _cmd.velocity.y + _cmd.velocity.z * _cmd.velocity.z);
  acc = sqrt(_cmd.acceleration.x * _cmd.acceleration.x + _cmd.acceleration.y * _cmd.acceleration.z + _cmd.acceleration.z * _cmd.acceleration.z);
  if(vel > max_vel)
  {
    max_vel = vel;
    //std::cout << "cmd_max vel: " << max_vel << std::endl;
    //std::cout << "cmd_max acc: " << max_acc << std::endl;
  }
  if(acc > max_acc)
  {
    max_acc = acc;
...
void cmdCallback(const ros::TimerEvent &e)
{
  ...
  cmd_position = Eigen::Vector3d(_cmd.position.x, _cmd.position.y, _cmd.position.z);
  // cmd_q = Eigen::Quaterniond(_cmd_q.w, _cmd_q.x, _cmd_q.y, _cmd_q.z);
...
int main(int argc, char **argv)
{
  ...
  /*加入的控制cmd话题发布频率*/
  // double cmd_pub_rate;
  // double cmd_pub_duration;

  node.param("rate/cmd", cmd_pub_rate, 100.0);
  // cmd_pub_duration = ros::Duration(1 / cmd_pub_rate);
  // const ros::Duration cmd_pub_duration(1 / cmd_pub_rate);
  // cmd_pub_duration = 1 / cmd_pub_rate;
