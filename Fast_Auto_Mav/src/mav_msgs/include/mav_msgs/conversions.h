...
inline void eigenTrajectoryPointFromMsg(
  ...
  // trajectory_point->timestamp_ns = msg.header.stamp.toNSec();
...
// Convenience method to quickly create a trajectory from a single waypoint.
inline void msgMultiDofJointTrajectoryFromPositionVelAccYaw(
    const Eigen::Vector3d& position, const Eigen::Vector3d& vel,const Eigen::Vector3d&acc, double yaw,
    trajectory_msgs::MultiDOFJointTrajectory* msg) {
  assert(msg != NULL);
