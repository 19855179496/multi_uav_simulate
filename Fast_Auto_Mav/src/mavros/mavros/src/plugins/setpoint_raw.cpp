...
private:
	...
	void rpyt_cb(const mav_msgs::RollPitchYawrateThrustConstPtr msg) {
      ...
      uint8_t type_mask = mavros_msgs::AttitudeTarget::IGNORE_ROLL_RATE |
                    mavros_msgs::AttitudeTarget::IGNORE_PITCH_RATE |
                    mavros_msgs::AttitudeTarget::IGNORE_YAW_RATE;//1+2+4
	//   //打印pitch roll
	//   ROS_INFO_STREAM("msg->pitch " << msg->pitch);
	//   ROS_INFO_STREAM("msg->roll " << msg->roll);

      //geometry_msgs::Quaternion orientation = tf::createQuaternionMsgFromRollPitchYaw(msg->roll, msg->pitch, 0);
	  geometry_msgs::Quaternion orientation = tf::createQuaternionMsgFromRollPitchYaw(msg->roll, msg->pitch, msg -> yaw_rate);
      double thrust = std::min(1.0, std::max(0.0, msg->thrust.z * thrust_scaling_ * system_mass_kg_));

	//   //将四元数转换为欧拉角
	//   tf::Quaternion q(orientation.x, orientation.y, orientation.z, orientation.w);
	   Eigen::Vector3d euler;
	//   tf::Matrix3x3(q).getRPY(euler[0], euler[1], euler[2]);
	//   //打印欧拉角
	//   ROS_INFO_STREAM("euler000 " << euler[0] << " " << euler[1] << " " << euler[2]);

      Eigen::Quaterniond desired_orientation;
      Eigen::Vector3d body_rate;
      //tf::quaternionMsgToEigen(orientation, desired_orientation); 
	  desired_orientation = ftf::to_eigen(orientation);
      // Transform desired orientation to represent aircraft->NED,
      // MAVROS operates on orientation of base_link->ENU
		auto ned_desired_orientation = ftf::transform_orientation_enu_ned(
			ftf::transform_orientation_baselink_aircraft(desired_orientation));
	  //将四元数转换为欧拉角
	  tf::Quaternion q1(ned_desired_orientation.x(), ned_desired_orientation.y(), ned_desired_orientation.z(), ned_desired_orientation.w());
	  tf::Matrix3x3(q1).getRPY(euler[0], euler[1], euler[2]);
	  //打印欧拉角
	//   ROS_INFO_STREAM("euler111 " << euler[0] << " " << euler[1] << " " << euler[2]);

      body_rate.x() = 0;
      body_rate.y() = 0;
	  body_rate.z() = 0;
      //body_rate.z() = -yaw_rate_scaling_ * msg->yaw_rate;
	//   ROS_INFO_STREAM("msg->yaw_rate " << msg->yaw_rate);
	//   ROS_INFO_STREAM("body_rate.z() " << body_rate.z());
